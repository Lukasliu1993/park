...
: Node("freespace_planner", node_options), engage_(false)
{
  ...
  // NodeParam
  {
    ...
    p.th_arrived_distance_m = declare_parameter("th_arrived_distance_m", 0.2);
  ...
  // Subscribers
  {
    // route_sub_ = create_subscription<PoseStamped>(
    ...
    "~/input/engage", rclcpp::QoS{1}, std::bind(&FreespacePlannerNode::onEngage, this, _1));    
    twist_sub_ = create_subscription<TwistStamped>(
      "~/input/twist", 5, std::bind(&FreespacePlannerNode::onTwist, this, _1));
  }
...
void FreespacePlannerNode::onEngage(const autoware_vehicle_msgs::msg::Engage::ConstSharedPtr msg)
{
  engage_ = msg->engage;
  RCLCPP_INFO(rclcpp::get_logger("engage_"), "the engage_ is %d", engage_);
  // route_ = msg;

  
  goal_pose_.header.frame_id = "map";

  goal_pose_.pose.position.x = 22.635;
  goal_pose_.pose.position.y = 3.2389;
  goal_pose_.pose.position.z = 0.0;

  goal_pose_.pose.orientation.x = 0.0;
  goal_pose_.pose.orientation.y = 0.0;
  goal_pose_.pose.orientation.z = 0.99999;
  goal_pose_.pose.orientation.w = -0.0038168;
  // goal_pose_.pose.position.x = 15.9387;
  // goal_pose_.pose.position.y = -97.892;
  // goal_pose_.pose.position.z = 0.0;

  // goal_pose_.pose.orientation.x = -0.004312;
  // goal_pose_.pose.orientation.y = 0.0008637;
  // goal_pose_.pose.orientation.z = -0.028405;
  // goal_pose_.pose.orientation.w = 0.999587;
  auto yaw = tf2::getYaw(goal_pose_.pose.orientation);
  RCLCPP_INFO(rclcpp::get_logger("yaw"), "the yaw is %f rad, %f du", yaw, yaw * 180 * M_1_PI);

  const double delta_l = 0.2;
  const int fix_num = 20;
  const double Len = fix_num * delta_l;
  goal_pose_.pose.position.x = goal_pose_.pose.position.x + Len * std::cos(yaw);
  goal_pose_.pose.position.y = goal_pose_.pose.position.y + Len * std::sin(yaw);
  RCLCPP_INFO(rclcpp::get_logger("yaw"), "the astar goal is (%f,%f,0), angle:%f",
   goal_pose_.pose.position.x, goal_pose_.pose.position.y, yaw * 180 * M_1_PI);

  reset();
}


// void FreespacePlannerNode::onRoute(const PoseStamped::ConstSharedPtr msg)
// {
//   route_ = msg;
...
{
  twist_ = msg;
  // auto stamp_ = rclcpp::Time(twist_->header.stamp);
  // RCLCPP_INFO(rclcpp::get_logger("parking"), "twist_ origin stamp =  %f", stamp_.seconds());

  twist_buffer_.push_back(msg);
  // RCLCPP_INFO(rclcpp::get_logger("parking"), "push back twist msg");
  ...
  while (true) {
    ...
    // RCLCPP_INFO(rclcpp::get_logger("parking"), "twist_buffer pop front");
  }
}
...
bool FreespacePlannerNode::isStopped(const std::deque<TwistStamped::ConstSharedPtr> & twist_buffer,
  const double th_stopped_velocity_mps)
{
  // auto stamp_ = rclcpp::Time(twist_->header.stamp);
  // RCLCPP_INFO(rclcpp::get_logger("parking"), "twist_ stamp =  %f", stamp_.seconds());
  for (const auto & twist : twist_buffer) {
    // auto stamp = rclcpp::Time(twist->header.stamp);
    // RCLCPP_INFO(rclcpp::get_logger("parking"), "twist stamp =  %f", stamp.seconds());
...
void FreespacePlannerNode::extend_fix_trajectory()
{
  //#TODO :QIANFENG extend trajectory for last fixed path,
  //use a param function maybe better.
  auto yaw = tf2::getYaw(goal_pose_.pose.orientation);
  const double delta_l = 0.2;
  const int fix_num = 20;
  for (int i = 0; i < fix_num; i++) {
    double len = (i + 1) * delta_l;
    TrajectoryPoint fix_point;
    fix_point.accel = Accel();
    fix_point.twist = Twist();
    fix_point.twist.linear.x = -1.62 * (1.0 - i / 20.0) / 3.6 - 0.38 / 3.6;
    // if (i > 16) {
    //   fix_point.twist.linear.x = -0.72 / 3.6;
    // }
    fix_point.pose.orientation = goal_pose_.pose.orientation;
    fix_point.pose.position.x = goal_pose_.pose.position.x - len * std::cos(yaw);
    fix_point.pose.position.y = goal_pose_.pose.position.y - len * std::sin(yaw);
    RCLCPP_INFO(rclcpp::get_logger("aa"), "park goal is (%f,%f), rate is %f", fix_point.pose.position.x, 
    fix_point.pose.position.y,fix_point.twist.linear.x);
    fix_point.pose.position.z = goal_pose_.pose.position.z;
    trajectory_.points.push_back(fix_point);
...
void FreespacePlannerNode::updateTargetIndex()
{
  ...
  // RCLCPP_INFO(rclcpp::get_logger("parking"), "twist_buffer_ size is %d", static_cast<int>(twist_buffer_.size()));

  const auto is_stopped = isStopped(twist_buffer_, node_param_.th_stopped_velocity_mps);
  if (is_stopped) {
    RCLCPP_INFO(rclcpp::get_logger("parking"), "stopped");
  }
...
{
  // RCLCPP_INFO(rclcpp::get_logger("parking"), "on timer1111111111111");
  // if (!occupancy_grid_ ) {
  //   RCLCPP_INFO(rclcpp::get_logger("parking"), "no occupancy_grid_");
  // }
  // if (!engage_ ) {
  //   RCLCPP_INFO(rclcpp::get_logger("parking"), "no engage_");
  // }
  // if (!twist_ ) {
  //   RCLCPP_INFO(rclcpp::get_logger("parking"), "no twist_");
  // }
  ...
  // if (!occupancy_grid_ || !route_ || !engage_ || !twist_) {
  //   return;
  // }
  ...
  // }
  if (is_completed_) {
     RCLCPP_INFO(rclcpp::get_logger("parking"), "completed");
    return;
  }
  // Get current pose
  ...
  if (current_pose_.header.frame_id == "") {
    RCLCPP_INFO(rclcpp::get_logger("parking"), "no current_pose_");
    return;
  }
  ...
  if (isPlanRequired()) {
    ...
    // Plan new trajectory
    planTrajectory();
    extend_fix_trajectory();
  }

  // StopTrajectory
  if (trajectory_.points.size() <= 1) {
    RCLCPP_INFO(rclcpp::get_logger("parking"), "stop trajectory ");
    return;
  }
...
void FreespacePlannerNode::planTrajectory()
{
  ...
  if (result) {
    ...
    RCLCPP_INFO(rclcpp::get_logger("parking"), "trajectory size is %d", static_cast<int>(trajectory_.points.size()));
    for(int i = 0; i < static_cast<int>(trajectory_.points.size()); i++){
      RCLCPP_INFO(rclcpp::get_logger("bb"), "point %d park goal is (%f,%f), rate is %f", i, trajectory_.points[i].pose.position.x, 
      trajectory_.points[i].pose.position.y,trajectory_.points[i].twist.linear.x);
    }
