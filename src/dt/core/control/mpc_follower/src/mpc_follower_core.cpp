...
: Node("mpc_follower", node_options), tf_buffer_(this->get_clock()), tf_listener_(tf_buffer_)
{
  ...
  sub_engage_ = create_subscription<autoware_vehicle_msgs::msg::Engage>(
    "input/engage", rclcpp::QoS{1}, std::bind(&MPCFollower::onEngage, this, _1));
  /* for debug */
  ...
  // pub_debug_values_ =
  //   create_publisher<autoware_debug_msgs::msg::Float32MultiArrayStamped>("~/debug/debug_values",
  //   1);
...
void MPCFollower::onEngage(const autoware_vehicle_msgs::msg::Engage::ConstSharedPtr msg)
{
  engage_ = msg->engage;
}
...
bool MPCFollower::calculateMPC(autoware_control_msgs::msg::ControlCommand * ctrl_cmd)
{
  ...
  //   d.data.push_back(ctrl_cmd->velocity);                                // [9] command
  //   velocities d.data.push_back(current_velocity_ptr_->twist.linear.x);             // [10]
  //   measured velocity d.data.push_back(curr_v * tan(steer_cmd) / W);       // [11] angvel from
  //   steer command d.data.push_back(curr_v * tan(mpc_data.steer) / W);  // [12] angvel from
  //   measured steer d.data.push_back(curr_v * nearest_smooth_k);         // [13] angvel from path
  //   curvature d.data.push_back(nearest_smooth_k);      // [14] nearest path curvature (used for
  //   feedforward) d.data.push_back(nearest_k);             // [15] nearest path curvature (not
  //   smoothed) d.data.push_back(ps);                    // [16] predicted steer
